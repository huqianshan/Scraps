...
//std::cout<<"lfclht constructor debug line 3" << std::endl;
	...
	set_thread_num(size_type num)
	{
		if(thread_num > 0)
		{
			...
			delete_persistent<persistent_ptr<clht_meta>[]> (tmp_meta, thread_num);
...
#ifdef LFCLHT_DEBUG
		thread_logs.resize(thread_num);//vector重新调整大小
	...
	// Setup persistent buffers according to the thread_num.
		tmp_meta = make_persistent<persistent_ptr<clht_meta>[]>(thread_num);
...
//            int resize = 0;
            if (likely(bucket->next == nullptr))
            {
                if (unlikely(empty == nullptr))
                {
					...
					if(CAS(&(bucket->next.off), /*nullptr*/0, b_new.raw())) {//将nullptr改成数值0
