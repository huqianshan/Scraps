...
#else
#include <libpmemobj++/shared_mutex.hpp> //C++17
#endif
...
#if !LIBPMEMOBJ_CPP_USE_TBB_RW_MUTEX
using internal::shared_mutex_scoped_lock;//C++17ÌØÐÔ
#endif
...
public:
	...
	using scoped_t = tbb::spin_rw_mutex::scoped_lock;//·¶Î§Ëø
#else
...
*/
	struct segment
	{
			...
			size_type value_len, size_t loc, size_t key_hash, pool_base &pop)
		{
			...
			size_type rc=1;
			scoped_t lock_segment(segment_lock, false);//trueï¼šwrite lockï¼Œfalse: read lock
			for(size_type i=0;i<kNumPairPerCacheLine*kNumCacheLine;i++)
			{
				auto slot_idx = (loc + i) % KNumSlot;
				scoped_t lock_bucket(bucket_lock[slot_idx],true);//trueï¼šwrite lockï¼Œfalse: read lock
...
*/
	...
	ret get(const key_type &key, size_type key_len)
	{
		...
		scoped_t lock_target_segment(target_segment->segment_lock, false);//trueï¼šwrite lockï¼Œfalse: read lock
		for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i)
		{
