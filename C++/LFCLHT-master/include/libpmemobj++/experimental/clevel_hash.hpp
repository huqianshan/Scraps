...
namespace pmem
{
		...
		namespace experimental
		{
			...
			public:
...
#if LIBPMEMOBJ_CPP_USE_TBB_RW_MUTEX
				using mutex_t = pmem::obj::experimental::v<tbb::spin_rw_mutex>; //tbb是什么？
								...
								thread_num(0)
				{
					...
					expand_bucket = 0;										 //当前迁移的bucket号
					expand_thread = std::thread(&clevel_hash::resize, this); //扩容线程！
	 ...
	 */
				...
				capacity(level_meta_ptr_t m_copy) const
				{
					...
					{
						level_bucket *cl = li.get_address(my_pool_uuid); //cl是current level的缩写
	 ...
	 */
				...
				uint64_t &level_num, level_meta_ptr_t &m_copy)
			{
				...
				while (true)
				{
					...
					*e = nullptr;
					...
					// Context checking.
					if (m_copy == meta) //判断两个指针是否指向相同的对象地址
					{
						return result;
					}
					else
				...
				size_type thread_id, level_meta_ptr_t &m_copy)
			{
				...
				{
				RETRY_FIND:
					level_meta *m = static_cast<level_meta *>(m_copy(my_pool_uuid)); //强制类型转换，指向同一个对象地址
					*e = nullptr;
					...
					{
						cl = levels[i].get_address(my_pool_uuid); //level_ptr_t需要通过get_address函数获得具体的指针
						f_idx = first_index(hv, cl->capacity);
						s_idx = second_index(partial, f_idx, cl->capacity);
...
#endif
					level_meta_ptr_t m_copy(meta); //拷贝构造
					pop.persist(&(meta.off), sizeof(uint64_t));
				...
				pool_base &pop, size_type thread_id, level_meta_ptr_t m_copy)
			{
				...
				if (cl->up == nullptr)
				{
					...
					pop.persist(&(cl->up.off), sizeof(uint64_t)); //有必要吗？
					...
					while (true)
					{
						...
						{
							m_copy = level_meta_ptr_t(meta); //拷贝赋值一份！
				...
				else
				{
					...
					if (meta == m_copy)
					{
						...
						while (true)
						{
							...
							{
								m_copy = level_meta_ptr_t(meta); //不等时写时拷贝一份
								m = static_cast<level_meta *>(m_copy(my_pool_uuid));
								cl = m->first_level.get_address(my_pool_uuid);

								if (cl->capacity >= new_capacity & m->is_resizing) //前一个条件想表达什么？
								{
									// CAS fails because other threads help updating meta
									delete_persistent_atomic<level_meta>(tmp_meta[t_id]);
									break;
			...
			clevel_hash<Key, T, Hash, KeyEqual, HashPower>::resize()
			{
				...
				{
					level_meta_ptr_t m_copy(meta); //浅拷贝构造，只复制指针，可能造成double free错误！
					pop.persist(&(meta.off), sizeof(uint64_t));
					...
					{
						usleep(10000); //sleep 10000us=10ms
						continue;
					}
					...
					{
					RETRY_REHASH:
						m_copy = level_meta_ptr_t(meta); //拷贝一份元数据context
						pop.persist(&(meta.off), sizeof(uint64_t));
						...
						if (static_cast<size_type>(expand_bucket) == bl->capacity)
						{
							...
							while (true)
							{
								...
								make_persistent_atomic<level_meta>(pop, tmp_meta[t_id],
																   m->first_level, bl->up, levels_left != 2); //这句没有弄懂
