...
main(int argc, char *argv[])
{
...

	{
		...
		proot->cons->set_thread_num(2);//这里为何为2？
	...
	thread_num--; // one thread reserved for background resizing这个才是核心对吗？如何设置resizing线程数
	...
	while(getline(&pbuf,&len,ycsb_read) != -1){
		if(strncmp(buf, "INSERT", 6) == 0){
			...
			run_queue[operation_num%thread_num][move[operation_num%thread_num]].operation = clevel_op::INSERT;//clevel_op为枚举类型
...
//真正建立threads数组vector
	...
	{
		threads.emplace_back([&](size_t thread_id) {//采用eplace_back替代了vector的push_back,优化了性能
			...
			for (size_t j = 0; j < READ_WRITE_NUM / thread_num; j++)
			{
				if (THREADS[thread_id].run_queue[j].operation == clevel_op::INSERT)
				{
					auto ret = map->insert(persistent_map_type::value_type(
						...
						THREADS[thread_id].run_queue[j].key),
					thread_id + 1, offset + j);//注意thread_id+1，是否可以加resizing_thread_num
					if (!ret.found)
					{
