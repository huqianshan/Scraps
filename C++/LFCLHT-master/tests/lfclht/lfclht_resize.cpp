...
#include "../profile.hpp"
#include <libpmemobj++/experimental/lf_clht.hpp> //适配修改源文件
...
int main(int argc, char *argv[])
{
...

	{
		...
		//以下是非常重要的设置，尤其对多线程而言，未设置的话极易造成段错误segment fault（core dumped）
	...
	fout = fopen("lfclht_exp2_load_factor.csv", "w");
	...
	while (getline(&pbuf, &len, ycsb) != -1)
	{
		...
		{ //按size比较两个字符串的关系
			...
			auto ret = map->put(persistent_map_type::value_type(key, key), 1, loaded); //适配修改添加thread_id为1.
																					   //test
