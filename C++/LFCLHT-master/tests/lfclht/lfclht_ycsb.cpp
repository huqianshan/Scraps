...
#include "../profile.hpp"
#include <libpmemobj++/experimental/lf_clht.hpp> 	//需要适配修改对应的源码文件

#define LAYOUT "lfclht" //需要适配修改
#define KEY_LEN 15
#define VALUE_LEN 16
#define LATENCY_ENABLE 1 //适配修改，增加延迟的测试。

#ifdef MACRO_TEST_FOR_LFCLHT
...
main(int argc, char *argv[])
{
...

	{
		...
		proot->cons->set_thread_num(2);//lock_free setup is necessary 
	...
	while (getline(&pbuf, &len, ycsb) != -1) {
		if (strncmp(buf, "INSERT", 6) == 0) {
			...
			auto ret = map->put(persistent_map_type::value_type(key, key), 1, loaded);//适配修改，添加thread_id为1.
			if (!ret.found) {
				loaded++;
			} else {
				break;
...
//相对clht测试独立增加的结束部分
	...
	while(getline(&pbuf,&len,ycsb_read) != -1){
		if(strncmp(buf, "INSERT", 6) == 0){
			...
			run_queue[operation_num%thread_num][move[operation_num%thread_num]].operation = lfclht_op::INSERT;//进行适配修改
			...
			run_queue[operation_num%thread_num][move[operation_num%thread_num]].operation = lfclht_op::READ;
			move[operation_num%thread_num] ++;
		}
		else if (strncmp(buf, "DELETE", 6) == 0){
			run_queue[operation_num%thread_num][move[operation_num%thread_num]].key = string_t(buf+7, KEY_LEN);
			run_queue[operation_num%thread_num][move[operation_num%thread_num]].operation = lfclht_op::DELETE;
			move[operation_num%thread_num] ++;
		}
		operation_num ++;
	}
	...
	for (size_t i = 0; i < thread_num; i++)
	{
		threads.emplace_back([&](size_t thread_id) {
			...
			{
				if (THREADS[thread_id].run_queue[j].operation == lfclht_op::INSERT)
				{
					auto ret = map->put(persistent_map_type::value_type(
						THREADS[thread_id].run_queue[j].key,
						THREADS[thread_id].run_queue[j].key), thread_id, offset + j);//需要适配修改添加了thread_id
					if (!ret.found)
					{
				...
				else if (THREADS[thread_id].run_queue[j].operation == lfclht_op::READ)
				{
					auto ret = map->get(persistent_map_type::key_type(
						THREADS[thread_id].run_queue[j].key));//需要适配修改
					if (ret.found)
					{
				...
				else if (THREADS[thread_id].run_queue[j].operation == lfclht_op::DELETE)
				{
					auto ret = map->erase(persistent_map_type::key_type(
						THREADS[thread_id].run_queue[j].key), thread_id);//需要适配修改添加了thread_id
    ...
    FILE *fp_time = fopen("lfclht_processing_time.txt", "w");
    for (size_t t = 0; t < thread_num; ++t)
    {
	...
	total_latency = 0;
	FILE *fp_latency = fopen("lfclht_latency.txt", "w");
	for (size_t t = 0; t < thread_num; ++t)
	{
