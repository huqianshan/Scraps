...
public:
	...
	{
		return hash(str.c_str(), str.size());//字符串转成c风格的字符串c_str()
	}
...
main(int argc, char *argv[])
{
	...
	while (getline(&pbuf, &len, ycsb) != -1) {//ssize_t getline(char **lineptr, size_t *n, FILE *stream);
		if (strncmp(buf, "INSERT", 6) == 0) {
			...
			if (loaded % 10000 == 0)
			{
				...
				printf("loaded %ld items, #buckets %ld, size %ld, #capacity %ld, load factor %f\n",
