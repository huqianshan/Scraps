...
#include "../profile.hpp"
#include <libpmemobj++/experimental/level_hash.hpp>//需要适配修改
...
main(int argc, char *argv[])
{
			...
			for (size_t j = 0; j < READ_WRITE_NUM / thread_num; j++)
			{
				if (THREADS[thread_id].run_queue[j].operation == level_hash_op::INSERT)
				{
					auto ret = map->insert(persistent_map_type::value_type(
						...
						THREADS[thread_id].run_queue[j].key), thread_id);//需要仔细设计，为什么thread_id不加1
					if (!ret.found)
					{
    ...
    FILE *fp_time = fopen("level_hash_tbb_processing_time.txt", "w");//需要适配修改以下4行level_hash字眼
