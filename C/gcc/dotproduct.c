...
void demo(size_t number)
{
    ...
    float *x1 = (float *)aligned_alloc(64, sizeof(float) * number);
    float *x2 = (float *)aligned_alloc(64, sizeof(float) * number);
    ...
    expected = dot512fma2_align(x1, x2, number);
    BEST_TIME(dot512fma2_align(x1, x2, number), expected, , repeat, number, bytes, true);
#endif
    printf("\n");
    free(x1);
    free(x2);
